...
async function prepareBataille() {
  ...
  const zonesDep = zonesDeploiement(dp, 14, 10);
  const terrain = genererTerrain({ cols: 14, rows: 10, biome, deploiement: zonesDep });
...
function terrainBloc(terrain) {
  ...
  const depLbl = (terrain.deploiement && terrain.deploiement.libelle) ? " · " + esc(terrain.deploiement.libelle) : "";
  return '<div class="cp-terrain">' +
    '<div class="cp-terrain-head">' +
      '<span class="cp-terrain-titre">Terrain de bataille</span>' +
      '<span class="cp-terrain-sub">' + terrain.cols + "×" + terrain.rows + " · " + esc(bioLbl) + depLbl +
