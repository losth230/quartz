
<center><h2>Règles de Faction</h2></center>

#### Gel
Les modèles gelés souffrent d’un malus de -1 à leurs jets d’armure. Cumulable.
#### Robustesse glaciale (1)
À utiliser lorsque vous activez un modèle *Fatigué*, il perd sa fatigue et reçoit [[Wargame/_Visualisation/Règles spéciales/Egide|Egide (2)]].
#### Transit
Chaque fois qu’un joueur active un modèle, celui-ci doit jeter un dé, si la valeur est supérieure à sa vitesse, le modèle est marqué Fatigué.

---

<center><h2>Domaines de Faction</h2></center>

<center><h3>Domaine du Givre</h3></center>

#### Niveau 1
- **Morsure de l’hiver (long, 7+, projectile) :** Portée 12’’ 5d6. Si la moindre blessure est obtenue, la cible est *Fatiguée*.
- **Tenir bon ! (court, 3+, amélioration) :** Portée 9’’, cible un modèle ami, même si celui-ci est engagé en mêlée. Il bénéficie d’un bonus de +1 à ses jets de moral et d’armure pour le reste du tour.
→ Version renforcée : au prix d’une action longue, le bonus aux jets passe de +1 à +2.
#### Niveau 2
- **Blizzard (long, 8+, météo) :** La vitesse de tous les modèles est réduite de 1, et leur vitesse ne peut pas dépasser 5.
- **Mur de glace (long, 8+, projectile) :** Placez un mur de glace entièrement à 18’’ du lanceur. Il fait Taille 4 et est indestructible. Il disparait à la fin du tour.
#### Niveau 3
- **[[Wargame/_Visualisation/Règles spéciales/Souffle|Souffle]] du grand dragon blanc (court, 13+, projectile) :** Portée 12’’, 3d6, Gel, [[Wargame/_Visualisation/Règles spéciales/Souffle|Souffle]]. Ce sort se lance deux fois.
→ Version renforcée (2) : augmente la portée du sort de 3’’ et lance un trait supplémentaire.
---
## Commandants
### Fine-lame — 36 pts

| ⬜ | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Fine-lame | **6** | **3+** | **-** | **5+** | **9** | **4** | **2** | **3** | Norls |
| **Mêlée:** | 5d6 |  | **Tir:** | - |  |  |  |  |  |

*[[Wargame/_Visualisation/Règles spéciales/Furtif|Furtif]], [[Wargame/_Visualisation/Règles spéciales/Force Dévastatrice|Force Dévastatrice (1)]].*
##### Équipement :
- Lame de glace : si cette unité réussit une blessure en mêlée, sa cible est marquée Gelée.
---

<div class="legendary">

### Jörgen — 45 pts

</div>

| ⬜⬜ | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Sabaoth | **6** | **3+** | **-** | **5+** | **10** | **4** | **3** | **4** | Norls |
| **Mêlée:** | 6d6 |  | **Tir:** | - |  |  |  |  |  |

*[[Wargame/_Visualisation/Règles spéciales/Anti-Infanterie|Anti-Infanterie]], [[Wargame/_Visualisation/Règles spéciales/Force Dévastatrice|Force Dévastatrice (1)]], [[Wargame/_Visualisation/Règles spéciales/Furtif|Furtif]].*
##### Équipement :
- Hache de Fordinson : si cette unité réussit une blessure en mêlée, ajoutez des marqueurs fatigue ou gel à la cible de manière qu’elle ait autant de fatigues que de gel.
---

<div class="legendary">

### Daïne Cœur-de-Glace — 49 pts

</div>

| ⬜⬜ | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Daïne | **4** | **6+** | **3+** | **5+** | **10** | **2** | **2** | **4** | Norls |
| **Mêlée:** | 3d6 |  | **Tir:** | - |  |  |  |  |  |

##### Magie :
- Daïne est un lanceur de sorts de niveau 4 à accès au Domaine des Cieux et au Domaine du Givre.
- Invocation élémentaire (long, 10+, amélioration) : Daïne peut invoquer une tempête de glace sous ses pieds. Elle ne peut alors plus se déplacer mais possède Ta 6 et la portée de toutes ses sorts, ses aptitudes et leurs effets est augmentée de 4’’.
- Transfert (court, 3+, amélioration) : Daïne se téléporte sur 3’’.
##### Règles spéciales :
- Cœur-de-Glace : Daïne ne peut perdre plus de 1 PV par tour.
- Hiver sans fin : au début de chaque tour, ajoutez autant de marqueurs Gelé que le numéro du tour, répartis comme vous le souhaitez parmi les modèles à 12’’ ou moins.
---
## Modèles de base
---
### Berserker — 17 pts

|  | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Berserker | **5** | **4+** | **-** | **5+** | **8** | **2** | **2** | **2** | Norls |
| **Mêlée:** | 3d6 |  | **Tir:** |  |  |  |  |  |  |

*[[Wargame/_Visualisation/Règles spéciales/Maitre-Lame|Maitre-Lame]].*
##### Équipement :
- Paire de haches (+3 pts) : confère +1d6 en mêlée.
##### Règles spéciales :
- Briser la glace : les attaques de ce modèle sont explosives sur 5+ si la cible est Gelée.
---
### Guerrier des steppes — 6 pts

|            | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| ---------- | ------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ------ | ------ | ------ | -------- |
| Guerrier   | **5**   | **5+**                                                                               | **-**                                                                                   | **5+**                                                                          | **7**                                                                            | **1**  | **2**  | **1**  | Norls    |
| **Mêlée:** | 2d6     |                                                                                      | **Tir:**                                                                                | -                                                                               |                                                                                  |        |        |        |          |

##### Équipement :
- Bouclier (+1 pts) : permet de relancer les jets d’armure de 1 venant de l’arc frontal.
---
### Porteur-de-Givre — 21 pts

|  | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Cavalier | **-** | **5+** | **-** | **5+** | **8+** | **2** | **3** | **2** | Norls |
| Cheval | **6** | **4+** | **-** | **-** | **-** | **-** | **-** | **-** | Bête |
| **Mêlée:** | 2d6 / 2d6 |  | **Tir:** | - |  |  |  |  |  |

*/ [[Wargame/_Visualisation/Règles spéciales/Cavalerie|Cavalerie]], [[Wargame/_Visualisation/Règles spéciales/Force Dévastatrice|Force Dévastatrice (1)]].*
##### Règles spéciales :
- Ruée de l’hiver : cette unité commence la partie Gelé. S’il obtient la moindre touche lors d’une charge, sa cible est marquée fatiguée.
- Râle tétanisant : lorsque ce modèle meurt, désignez un modèle à 4’’ ou moins à qui transférer les marqueurs Gelés.
---
## Modèles de soutient
---
### Evocateur — 31 pts

| 🟥 | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Evocateur | **5** | **6+** | **4+** | **5+** | **7** | **2** | **2** | **4** | Norls |
| **Mêlée:** | 2d6 |  | **Tir:** | - |  |  |  |  |  |

##### Magie :
- L’Evocateur est un lanceur de sorts de niveau 3 à accès au Domaine des Cieux et au Domaine du Givre.
##### Règles spéciales :
- Surcharge élémentaire (1/3 par unité au sein d’un modèle) : au prix d’une action courte, vous pouvez renforcer un modèle ami à 6’’ ou moins avec l’une des améliorations suivantes :
  - Froid mordant : jusqu’à la fin de la partie, les modèles attaquées par le modèle sont marqués Fatigués, ou Activés s’ils l’étaient déjà.
  - Passage de l’hiver : jusqu’à la fin de la partie, ce modèle peut effectuer une marche gratuite au début de chaque tour.
---
## Modèles Spéciaux
---
### Elémentaire de givre — 58 pts

|  | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Elémentaire | **4** | **4+** | **-** | **4+** | **9** | **8** | **4** | **5** | Elémentaire |
| **Mêlée:** | spé |  | **Tir:** | - |  |  |  |  |  |

*[[Wargame/_Visualisation/Règles spéciales/Force Dévastatrice|Force Dévastatrice (3)]], [[Wargame/_Visualisation/Règles spéciales/Régénération|Régénération (1)]].*
##### Règles spéciales :
- Elémentaire de glace : cette unité ne peut être blessée que par des attaques ayant Force Dévastatrice ou [[Wargame/_Visualisation/Règles spéciales/Perforant|Perforant]]. De plus, cette unité est immunisée au *Gel*.
- Frappe de glace : la compétence en mêlée de cette unité est égale au nombre de marqueur Gel qu’elle possède.
- Cœur gelé : ce modèle commence avec 2 marqueurs Gel.
---
### Grand dragon blanc — 108 pts

|  | **Vit** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque en mêlée\|Me]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer une attaque à distance\|Tir]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un jet d'armure\|Ar]]** | **[[Wargame/_Visualisation/Règles de bataille/Effectuer un test de moral\|Mo]]** | **PV** | **Ta** | **Ma** | **Type** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Dragon | **6** | **3+** | **4+** | **3+** | **10** | **12** | **5** | **8** | Bête |
| **Mêlée:** | 10d6 |  | **Tir:** | - |  |  |  |  |  |

*[[Wargame/_Visualisation/Règles spéciales/Anti-Infanterie|Anti-Infanterie]], [[Wargame/_Visualisation/Règles spéciales/Cavalerie|Cavalerie]], [[Wargame/_Visualisation/Règles spéciales/Force Dévastatrice|Force Dévastatrice (2)]].*
##### Équipement :
- [[Wargame/_Visualisation/Règles spéciales/Souffle|Souffle]] de grand dragon blanc : Portée 12’’, 3d6, [[Wargame/_Visualisation/Règles spéciales/Souffle|Souffle]]. Ceci est une attaque à distance utilisable même en mêlée, qui se lance deux fois (vous pouvez pivoter entre chaque attaque).
##### Règles spéciales :
- Ecailles de dragon : cette unité ne peut être blessée que par des attaques ayant Force Dévastatrice ou [[Wargame/_Visualisation/Règles spéciales/Perforant|Perforant]].
---